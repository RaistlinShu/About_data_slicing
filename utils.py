...
def create_bins(lower_bound, upper_bound, quantity):
    ...
    for i in range(quantity - 1):
        bins.add((int(lower_bound + i * width), int(lower_bound + (i + 1) * width)))
    bins.add((int(lower_bound + (quantity - 1) * width), upper_bound + 1))
    return bins
...
def divide_slice(data):
    res = {}
    data_size = data.shape[0]
    size_threshold = data_size / 100
    for column in data:
